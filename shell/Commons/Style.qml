...
QtObject {
  ...
  // component follows the fontconfig alias `roseshell-font-set` writes.
  ...
  // alias path keeps working when the user runs `roseshell font set`), but
  ...
  // ROSESHELL_MENU_FONT override for users who want a different family on the
  ...
  readonly property string menuFontFamily: {
    var override = Quickshell.env("ROSESHELL_MENU_FONT")
  ...
  // Resolve the fontconfig alias to a concrete family name. `roseshell font
  ...
  // `roseshell toggle window-gaps` creates/removes this flag file. Hyprland
  ...
  property FileView windowNoGapsToggle: FileView {
    path: Quickshell.env("HOME") + "/.local/state/roseshell/toggles/hypr/window-no-gaps.lua"
    watchChanges: true
    printErrors: false
