...
QtObject {
  ...
  readonly property string currentThemePath: stateHome + "/roseshell/current/theme"
  ...
  // Style. User keys win, so a machine-level `~/.config/roseshell/shell.toml`
  ...
  // is where `roseshell display text size` writes `[font] base-size`. Watched so the
  // CLI takes effect live without restarting the shell; absent by default.
  property FileView userShellFile: FileView {
    id: userShellFile
    path: root.home + "/.config/roseshell/shell.toml"
    watchChanges: true
    printErrors: false
