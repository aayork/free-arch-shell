...
Item {
  ...
  function dismiss() {
    ...
    if (root.shell && typeof root.shell.hide === "function")
      root.shell.hide((root.manifest && root.manifest.id) || "roseshell.emojis")
  }
  ...
  function applySelected(emoji) {
    if (!emoji) return
    root.dismiss()
    Quickshell.execDetached([root.roseshellPath + "/bin/roseshell-menu-emoji-insert", emoji])
  }

  ListModel { id: displayModel }

  FileView {
    path: root.roseshellPath + "/shell/plugins/emojis/emojis.json"
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "roseshell-emojis"
