...
// roseshell-agent-usage-update, which writes one JSON record per agent into
...
Item {
  ...
  readonly property string usageDir: (Quickshell.env("XDG_STATE_HOME") || home + "/.local/state") + "/roseshell/agents/usage"
  ...
  function updateCommand(kind, agentIds) {
    var command = ["roseshell-agent-usage-update"]
  ...
  readonly property string syncSnapshotPath: syncConfigured() ? syncEffectiveDir + "/" + syncEffectiveFileName : home + "/.cache/roseshell/agents-disabled.json"
  ...
  // Snapshots keep the field names older Roseshell versions wrote, so a fleet
