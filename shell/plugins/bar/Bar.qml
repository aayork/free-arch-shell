...
Item {
  ...
  // The roseshell-shell host injects roseshellPath from ROSESHELL_PATH.
  ...
  property string roseshellConfigDir: home + "/.config/roseshell"
  ...
  // to "monospace"), so changing the system font (via `roseshell-font-set`)
  ...
  // Resolve the live bar-widget instance for a plugin id (e.g. "roseshell.bluetooth").
  ...
  function customModuleSource(entry) {
    var source = BarModel.customModulePath(entry, home, roseshellConfigDir)
  ...
  FileView {
    path: root.stateHome + "/roseshell/current"
    watchChanges: true
    printErrors: false
  ...
  // and the flag is created/removed by `roseshell-toggle-bar`.
  Process {
    id: barHiddenProbe
    running: true
    command: ["bash", "-c", "[[ -f $HOME/.local/state/roseshell/toggles/bar-off ]] && echo yes || echo no"]
    stdout: SplitParser { onRead: function(line) { root.barHidden = String(line).trim() === "yes" } }
  }
  FileView {
    path: root.home + "/.local/state/roseshell/toggles"
    watchChanges: true
    printErrors: false
  ...
  // shell restarts. `roseshell-toggle-bar` nudges this after flipping the flag
  // so the probe re-reads it even when the watch has gone quiet.
  IpcHandler {
    target: "roseshell.bar"
  ...
  component DragGhostPanel: PanelWindow {
    ...
    WlrLayershell.namespace: "roseshell-bar-drag-ghost"
  ...
  component BarMoveGhostPanel: PanelWindow {
    ...
    color: "transparent"
    exclusionMode: ExclusionMode.Ignore
    WlrLayershell.namespace: "roseshell-bar-move-ghost"
    WlrLayershell.layer: WlrLayer.Overlay
    WlrLayershell.keyboardFocus: WlrKeyboardFocus.None
