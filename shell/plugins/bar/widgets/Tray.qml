...
BarWidget {
  ...
  // UseQApplication` - roseshell's shell.qml does not, so every submenu click was
  ...
  function ownedByRoseshell(item) {
    var layout = root.bar && root.bar.layoutConfig ? root.bar.layoutConfig : null
    return TrayModel.ownedByRoseshell(item, layout)
  }
  ...
  function bucket(category) {
    ...
    for (var i = 0; i < values.length; i++) {
      ...
      if (ownedByRoseshell(item)) continue
      if (category === "all") {
        result.push(item)
        continue
      }
  ...
  function persistTrayState(pinned, hidden) {
    ...
    var id = root.moduleName || "roseshell.tray"
