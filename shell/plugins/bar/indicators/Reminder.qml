...
BarIndicator {
  ...
  function openReminderFlow() {
    Quickshell.execDetached(["roseshell-reminder", "-i"])
  }
  ...
  Process {
    id: jsonProc
    command: ["roseshell-reminder", "show", "--json"]
  ...
  onPressed: function() {
    if (root.reminderCount > 0) Quickshell.execDetached(["roseshell-reminder", "show"])
