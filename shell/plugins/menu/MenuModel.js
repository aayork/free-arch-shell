...
// against `roseshell-default-browser` -- so the batch runs it once and the rows
...
var GUARD_READERS = [
  "roseshell-channel-current",
  "roseshell-default-agent",
  "roseshell-default-browser",
  "roseshell-default-editor",
  "roseshell-default-terminal",
  "roseshell-dns"
]
...
function guardHelpers() {
  return 'declare -A __roseshell_pkgs=()\n'
    + 'mapfile -t __roseshell_pkg_names < <({ pacman -Qq; LC_ALL=C pacman -Qi'
    + " | awk '/^[A-Za-z]/ { provides = ($0 ~ /^Provides/); sub(/^[^:]*: /, \"\") }"
    + ' provides && $0 != "None" { n = split($0, p, " ");'
    + ' for (i = 1; i <= n; i++) { sub(/[<>=].*/, "", p[i]); print p[i] } }\'; } 2>/dev/null)\n'
    + 'for __roseshell_pkg in "${__roseshell_pkg_names[@]}"; do __roseshell_pkgs[$__roseshell_pkg]=1; done\n'
    + '__roseshell_pkg_has() { [[ -n ${__roseshell_pkgs[$1]-} ]] && return 0; '
    + '[[ $1 == *[\\<\\>=]* ]] && { pacman -Q "$1" &>/dev/null; return; }; return 1; }\n'
    + 'roseshell-pkg-present() { local p; for p in "$@"; do __roseshell_pkg_has "$p" || return 1; done; return 0; }\n'
    + 'roseshell-pkg-missing() { local p; for p in "$@"; do __roseshell_pkg_has "$p" || return 0; done; return 1; }\n'
    + 'roseshell-cmd-present() { local c; for c in "$@"; do command -v "$c" &>/dev/null || return 1; done; return 0; }\n'
    + 'roseshell-cmd-missing() { local c; for c in "$@"; do command -v "$c" &>/dev/null || return 0; done; return 1; }\n'
}
...
function guardPrelude(guards) {
  ...
  for (var i = 0; i < GUARD_READERS.length; i++) {
    ...
    prelude += "__roseshell_read_" + i + "=$(" + GUARD_READERS[i] + " 2>/dev/null) || :\n"
  }
...
function guardReaderSlot(index) {
  return "${__roseshell_read_" + index + "}"
}
