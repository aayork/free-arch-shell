...
Item {
  ...
  // Injected by roseshell-shell when this plugin is summoned.
  property string roseshellPath: Quickshell.env("ROSESHELL_PATH")
  property var shell: null
  property var manifest: null

  // Plugin lifecycle hooks. The host calls open(payloadJson) after
  // `roseshell-shell shell summon roseshell.menu ...` and close() when hidden.
  ...
  property string defaultMenuPath: roseshellPath + "/default/roseshell/roseshell-menu.jsonc"
  property string userMenuPath: Quickshell.env("HOME") + "/.config/roseshell/extensions/roseshell-menu.jsonc"
  ...
  readonly property var providers: ({
    "fonts": {
      script: "current=$(roseshell-font-current 2>/dev/null); roseshell-font-list 2>/dev/null | while read -r f; do [[ -z $f ]] && continue; printf '%s\\t%s\\t%s\\n' \"$f\" \"$f\" \"$current\"; done",
      icon: "",
      volatile: true,
      actionFor: function(value) { return "roseshell-font-set " + Util.shellQuote(value) }
    },
    "power-profiles": {
      script: "current=$(powerprofilesctl get 2>/dev/null); roseshell-powerprofiles-list 2>/dev/null | while read -r p; do [[ -z $p ]] && continue; printf '%s\\t%s\\t%s\\n' \"$p\" \"$p\" \"$current\"; done",
      icon: "\udb81\udc0b",
      actionFor: function(value) { return "roseshell-powerprofiles-set autodetect " + Util.shellQuote(value) }
    }
  })
  ...
  // `roseshell-shell shell summon roseshell.menu '{"menu":"system"}'`.
  ...
  function openRoute(initialMenu) {
    ...
    // a leaf, e.g. `roseshell menu summon screenrecord-stop`), run it directly
  ...
  // user extension at ~/.config/roseshell/extensions/roseshell-menu.jsonc) take
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "roseshell-menu"
