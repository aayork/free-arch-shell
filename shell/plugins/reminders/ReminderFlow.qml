...
Item {
  ...
  function dismiss() {
    ...
    if (root.shell && typeof root.shell.hide === "function")
      root.shell.hide((root.manifest && root.manifest.id) || "roseshell.reminders")
  }
  ...
  function submit() {
    ...
    if (root.step === "minutes") {
      ...
      if (!nextMinutes) {
        Quickshell.execDetached([root.roseshellPath + "/bin/roseshell-notification-send", "Invalid reminder", "Enter the number of minutes"])
        return
      }
    ...
    if (root.step === "message") {
      var args = [root.roseshellPath + "/bin/roseshell-reminder"].concat(ReminderFlowModel.reminderArgs(root.minutes, selection))
      root.dismiss()
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "roseshell-reminders"
