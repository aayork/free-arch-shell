...
// roseshell-disk-speedtest run streams both phases and cleans up after itself,
...
Item {
  ...
  function dismiss() {
    if (shell && typeof shell.hide === "function")
      shell.hide((manifest && manifest.id) || "roseshell.disk-speedtest")
    else close()
  }
  ...
  Process {
    id: proc
    command: ["roseshell-disk-speedtest"]
  ...
  SpeedTestOverlay {
    ...
    layerNamespace: "roseshell-disk-speedtest"
    title: root.diskName
    leftLabel: "READ"
    rightLabel: "WRITE"
    unit: "MB/s"
