...
Panel {
  id: root
  moduleName: "roseshell.bluetooth"
  ipcTarget: "roseshell.bluetooth"
  // manageIpc: false so this panel can own the single IpcHandler the target
  // permits — needed for the toggleBluetooth method below.
  manageIpc: false

  // Address -> "connecting" | "disconnecting" | "forgetting".
  // The actual Bluetooth sequencing lives in bin/roseshell-bluetooth-device;
  ...
  function setDefaultAudioSink(sink) {
    ...
    if (sink.id !== undefined && sink.name) {
      Quickshell.execDetached([
        "roseshell-audio-output-set-default",
        String(sink.id),
        String(sink.name)
      ])
    }
  ...
  function deviceCommand(action, address) {
    return ["roseshell-bluetooth-device", action, address]
  }
  ...
  // disconnects, then removes the BlueZ pairing record via roseshell-bluetooth-device.
  ...
  // the adapter came back on at the next boot. roseshell-bluetooth-power moves the
  ...
  function toggleBluetooth() {
    if (!adapter) return
    Quickshell.execDetached(["roseshell-bluetooth-power", adapter.enabled ? "off" : "on"])
  }

  IpcHandler {
    target: "roseshell.bluetooth"
