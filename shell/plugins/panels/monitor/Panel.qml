...
Panel {
  id: root
  moduleName: "roseshell.monitor"
  ipcTarget: "roseshell.monitor"
  manageIpc: false
  ...
  // these stops; the CLI (roseshell-display-text-size) accepts any integer in range.
  ...
  function setBrightness(value) {
    ...
    setBrightnessProc.command = ["roseshell-brightness-display", "--no-osd", "--monitor", root.focusedMonitor, percent + "%"]
  ...
  function showBrightnessOsd(percent) {
    ...
    bar.shell.summon("roseshell.osd", JSON.stringify({
      icon: "brightness",
      value: percent
    }))
  }
  ...
  function setScale(scale) {
    actionProc.command = ["bash", "-c", "roseshell-hyprland-monitor-scaling " + scale]
    if (!actionProc.running) actionProc.running = true
  }
  ...
  function setTextSize(px) {
    textScaleProc.command = ["roseshell-display-text-size", String(px)]
  ...
  Process {
    id: stateProc
    command: ["roseshell-monitor-state"]
  ...
  Process {
    ...
    // `roseshell-brightness-display` races the hardware/driver and can
