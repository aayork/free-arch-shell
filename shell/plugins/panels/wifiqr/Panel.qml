...
// roseshell-network-qr, which emits the interface, security, and SSID it
...
Item {
  ...
  function dismiss() {
    if (root.shell && typeof root.shell.hide === "function")
      root.shell.hide((root.manifest && root.manifest.id) || "roseshell.wifiqr")
    else close()
  }
  ...
  function generate(requestedIface) {
    ...
    qrProc.command = requestedIface
      ? ["roseshell-network-qr", "--meta", requestedIface]
      : ["roseshell-network-qr", "--meta"]
    qrProc.running = true
  }
  ...
  function togglePassword() {
    ...
    pwProc.command = ["roseshell-network-password", iface]
    pwProc.running = true
  }
  ...
  PanelWindow {
    ...
    WlrLayershell.namespace: "roseshell-network-qr"
