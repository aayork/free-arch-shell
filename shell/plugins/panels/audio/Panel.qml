...
Panel {
  id: root
  moduleName: "roseshell.audio"
  ipcTarget: "roseshell.audio"

  readonly property var sink: Pipewire.defaultAudioSink
  readonly property var source: Pipewire.defaultAudioSource
  readonly property var nodes: Pipewire.nodes ? Pipewire.nodes.values : []
  readonly property var mprisPlayers: Mpris.players ? Mpris.players.values : []
  readonly property var mediaService: bar?.shell?.firstPartyServiceFor("roseshell.media")
  ...
  readonly property var candidateStreams: {
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      if (String(n.name || "").indexOf("roseshell_speaker_tuning") === 0) continue
      list.push(n)
    }
    return list
  }
  ...
  //
  // roseshell-audio-output-sink resolves the *current* default output through any
  ...
  function setDefaultSource(node) {
    ...
    if (node.id !== undefined && node.name) {
      Quickshell.execDetached([
        "roseshell-audio-input-set-default",
        String(node.id),
        String(node.name)
      ])
    }
  ...
  Process {
    ...
    command: ["roseshell-audio-sink-availability"]
  ...
  Process {
    id: volumeSinkProc
    command: ["roseshell-audio-output-sink"]
    stdout: StdioCollector {
      waitForEnd: true
