...
// roseshell-network-status when the caller doesn't know it.
Item {
  id: root
  ...
  function dismiss() {
    if (root.shell && typeof root.shell.hide === "function")
      root.shell.hide((root.manifest && root.manifest.id) || "roseshell.speedtest")
    else close()
  }
  ...
  function startPhase(nextPhase) {
    ...
    phase = nextPhase
    stderrText = ""
    speedTestProc.command = ["roseshell-network-speedtest", nextPhase]
  ...
  Process {
    id: statusProc
    command: ["roseshell-network-status"]
  ...
  SpeedTestOverlay {
    ...
    layerNamespace: "roseshell-network-speedtest"
