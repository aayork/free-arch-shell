...
function glyphFromHints(hints) {
  return stringHint(hints, "roseshell-glyph")
}

// The click action: a JSON argv string from roseshell-notification-send
...
function execArgvFromHints(hints) {
  return stringHint(hints, "roseshell-exec-argv")
}

// Validate a persisted roseshell-exec-argv into a runnable argv, or null. This is
...
// ~/.local/state/roseshell/notifications/ so toasts survive shell restarts
// (e.g. the restart `roseshell-update` performs). The file exists exactly as
...
// senders (all Roseshell web apps) delete their scoped /tmp files on close,
