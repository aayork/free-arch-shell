// Notification service for the roseshell shell.
...
Item {
  ...
  readonly property string stateDir: home + "/.local/state/roseshell/"
  ...
  // Toasts are fixed to the top-right corner. They only clear the roseshell bar
  ...
  PersistentProperties {
    id: persisted
    reloadableId: "roseshell-notifications"
  ...
  // and rare.
  //   - roseshell-action: a user-action confirmation toast ("Theme changed",
  ...
  //     Trusted because it's almost always roseshell or system shell scripts —
  ...
  //   - app_name is "roseshell-action" (Roseshell's own user-action toasts —
  ...
  // Run the popup's click action, then dismiss. Roseshell's own toasts carry the
  ...
  function focusApp(entry) {
    ...
    focusAppProc.command = [
      service.roseshellPath + "/bin/roseshell-hyprland-focus-app",
      String(entry.app)
    ]
  ...
  // toasts survive shell restarts (notably the restart `roseshell-update`
  ...
  Variants {
    ...
    PanelWindow {
      ...
      WlrLayershell.namespace: "roseshell-notifications"
