...
Item {
  ...
  Process {
    id: laptopClosedProc
    command: ["bash", "-c", "roseshell-hw-laptop-closed && echo closed || echo open"]
  ...
  PolkitAgent {
    id: polkitAgent
    path: "/org/roseshell/PolkitAgent"
    ...
    onIsRegisteredChanged: {
      if (isRegistered) console.log("roseshell polkit agent registered")
      else console.warn("roseshell polkit agent is not registered; another agent may be running")
    }
  }
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "roseshell-polkit"
