...
Item {
  ...
  property string historyPath: Quickshell.env("HOME") + "/.local/state/roseshell/clipboard-history.json"
  property string captureScript: root.roseshellPath + "/shell/plugins/clipboard/capture.sh"
  ...
  function applySelected(row) {
    ...
    if (row.entryType === "image") {
      Quickshell.execDetached([root.roseshellPath + "/bin/roseshell-clipboard-paste-file", row.mime, row.path])
    } else if (row.fullText) {
      Quickshell.execDetached([root.roseshellPath + "/bin/roseshell-clipboard-paste-text", "--shift-insert", "--history-index", String(row.historyIndex)])
    }
  }
  ...
  function copySelected(row) {
    ...
    root.opened = false
    if (row.entryType === "image") {
      Quickshell.execDetached([root.roseshellPath + "/bin/roseshell-clipboard-paste-file", "--copy-only", row.mime, row.path])
    } else if (row.fullText) {
      Quickshell.execDetached([root.roseshellPath + "/bin/roseshell-clipboard-paste-text", "--copy-only", "--history-index", String(row.historyIndex)])
    }
  }
  ...
  function openSelected(row) {
    if (!row) return
    root.opened = false
    Quickshell.execDetached([root.roseshellPath + "/bin/roseshell-clipboard-open", "--history-index", String(row.historyIndex)])
  }
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "roseshell-clipboard"
