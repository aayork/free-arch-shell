...
QtObject {
  ...
  property string pluginsDir: home + "/.config/roseshell/plugins"
  ...
  function trustedCapabilities(manifest) {
    ...
    var metadata = Util.pluginMetadata(manifest)
  ...
  //   - the built-in bar option (`roseshell.bar`) is active when `bar.id` is
  //     missing or set to `roseshell.bar`.
  ...
  //     silently make `roseshell launch bar-settings` a no-op. Turning one off
  ...
  function isEnabled(id) {
    ...
    if (manifest) {
      if (Array.isArray(manifest.kinds) && manifest.kinds.indexOf("bar") !== -1) {
        ...
        if (!selectedBar) selectedBar = "roseshell.bar"
  ...
  // back, and so a plugin that is both a widget and a menu (roseshell.menu)
  ...
  function barTarget(config, placement, fallbackSection) {
    ...
    var anchors = { left: "roseshell.workspaces", center: "roseshell.weather", right: "roseshell.tray" }
  ...
  function activeCloneFor(config, sourceId) {
    for (var candidate in installedPlugins) {
      ...
      var candidateMetadata = candidateManifest && Util.pluginMetadata(candidateManifest)
  ...
  function restoreCloneSource(config, cloneId, sourceId) {
    ...
    if (isBarOption) {
      if (sourceId === "roseshell.bar") delete config.bar.id
    ...
    shellConfigMutator(function(config) {
      ...
      if (isBarOption) {
        if (value) {
          ...
          if (clonedFrom && clonedFrom !== "roseshell.bar") config.bar.id = clonedFrom
  ...
  function parseScanOutput(text) {
    ...
    // `roseshell.*` namespace is reserved for built-ins, including bar widgets
    // registered outside the manifest-based plugin registry.
    for (var tk in thirdParty) {
      if (firstParty[tk] || String(tk).indexOf("roseshell.") === 0) {
        console.warn("PluginRegistry: plugin " + tk
          + " rejected: id is reserved for first-party Roseshell plugins")
        continue
      }
  ...
  function rescan() {
    ...
    // Third-party plugins stay at the top level of ~/.config/roseshell/plugins.
    var script = ""
