...
`roseshell-shell` is a single long-running [Quickshell](https://quickshell.org/)
instance that hosts the Roseshell desktop. Hyprland autostart launches one shell
...
| `bar`        | A full bar option that can replace the built-in `roseshell.bar` |

Only one `bar` plugin is active at a time. Missing or invalid selections fall
back to the built-in `roseshell.bar`, so users always have a safe path home.
...
changed bar widget cannot destroy `roseshell.lock` while Hyprland still
...
clones it straight into `~/.config/roseshell/plugins/<id>/` (named by the
manifest id); updating is a fast-forward pull of that checkout.

```bash
roseshell plugin add https://github.com/acme/roseshell-weather.git
roseshell plugin update acme.weather       # fetches, shows a diff, fast-forwards
roseshell plugin update                    # updates every git-managed plugin
roseshell plugin remove acme.weather
```

> ⚠️ **Plugins run as unsandboxed code inside `roseshell-shell`.** Adding warns you before cloning, plugins land disabled so you can review the code before enabling, and updates show a diff of the changes before touching anything. The scoped QML interfaces remove direct authentication-service and generic replacement-bar service lookups, but visual plugins still share and can traverse the ordinary host scene. Only add repos whose code you are willing to run.
...
```bash
roseshell plugin add https://github.com/acme/roseshell-weather.git --enable --yes
roseshell plugin update --yes
```
...
1. Put it in `~/.config/roseshell/plugins/<plugin-id>/` with a `manifest.json`
   plus the QML referenced from its `entryPoints`.
2. `roseshell-shell shell rescanPlugins`.
3. `roseshell plugin enable <id>`. Bar widgets start in
   `barWidget.defaultSection`, or in the center when it is omitted, and can be
   moved with `roseshell bar move`; a full bar replaces the one in use.

The lower-level IPC equivalents remain available via `roseshell-shell shell rescanPlugins`,
`roseshell-shell shell enablePlugin <id> '{}'`, and `roseshell-shell shell listPlugins`.
The `roseshell plugin` commands wrap those calls. `roseshell bar move` and
`roseshell bar set` edit the persisted widget layout in `shell.json`.
...
Saving a file anywhere under `~/.config/roseshell/plugins/` reloads plugin code
automatically; `roseshell-shell shell rescanPlugins` remains available to force a reload.
...
hooks, the image picker's `image-selector` target). `roseshell-menu` uses the
shell target to summon the first-party `roseshell.menu` plugin instead of
...
| `reloadConfig`                           | `ok`    | reload `~/.config/roseshell/shell.json`                 |
...
```
quickshell ipc -p $ROSESHELL_PATH/shell call shell ping
```

Hyprland autostart launches the shell directly with `quickshell -p
$ROSESHELL_PATH/shell`. Use `roseshell-restart-shell` to stop every running
instance of that config and launch one fresh shell process.

A convenience wrapper, [`roseshell-shell`](../bin/roseshell-shell), forwards IPC
calls to the running shell. It does not start the shell.

```
roseshell-shell shell ping
roseshell-shell shell toggle roseshell.menu '{"menu":"root"}'
roseshell-shell shell listPlugins
roseshell-shell shell rescanPlugins
```
...
| Path                              | Owner          | Purpose                                                |
|-----------------------------------|----------------|--------------------------------------------------------|
| `~/.config/roseshell/shell.json`    | the shell      | full layout + per-entry settings + enabled plugin list |
| `~/.config/roseshell/plugins/<id>/` | user           | drop-in third-party plugin source files                |

The `config/roseshell/shell.json` default config describes the
...
```json
{
  ...
  "bar": {
    ...
    "layout": {
      ...
      "center": [ { "id": "roseshell.clock", "format": "HH:mm" } ],
      "right": [
        { "id": "roseshell.audio" }
      ]
    }
...
1. **The active bar option is `bar.id`.** Omit it or set it to `roseshell.bar`
...
4. **Built-in widget ids are namespaced.** Use ids such as `roseshell.clock`,
   `roseshell.audio`, and `roseshell.network`. The migration rewrites older ids
...
6. **Multiple instances** are allowed when a manifest sets
   ...
   widgets in different timezones are just two `{"id":"roseshell.clock", "timezone": ...}`
...
- Phase 1 — `roseshell-shell phase 1: host the existing bar in a single shell`
- Phase 2 — `roseshell-shell phase 2: plugin registry and bar widget registry`
- Phase 3 — `roseshell-shell phase 3: fold bar-settings into the shell as a panel plugin`
- Phase 4 — `roseshell-shell phase 4: absorb background-switcher as a plugin`
- Phase 5 — `roseshell-shell phase 5: docs, cleanup, and migration crumbs`
- Phase 6 — `roseshell-shell phase 6: reviewer cleanup (path traversal, collision, races)`
- Phase 7 — `roseshell-shell phase 7: replace socket with IpcHandler, rename to image-picker`
- Phase 8a — `roseshell-shell phase 8a: unified shell.json with inline plugin settings`
